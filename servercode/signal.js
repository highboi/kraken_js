	...
	ws.on("message", (message) => {
		...
		switch (messagedata.event) {
			case "join-net":
				...
				room[messagedata.userid] = {userid: messagedata.userid, socket: ws, attributes: messagedata.attributes};

				//send update to peers on network to update the dashboard of connected peers
				var peerjoinObj = JSON.stringify({peerid: messagedata.userid, peerattributes: messagedata.attributes, event: "join-net"});
				for (var peer of Object.keys(room)) {
					global.signalClients[peer].socket.send(peerjoinObj);
				}
